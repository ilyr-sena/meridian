"""Apple CA bundle helper to support Apple GrandSlam and Developer Services PKI."""

from __future__ import annotations

import logging
import sys
from pathlib import Path

log = logging.getLogger(__name__)


def _state_dir() -> Path:
    if sys.platform == "win32":
        import os
        appdata = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
        return Path(appdata) / "Meridian" / "state"
    return Path.home() / ".local" / "state" / "meridian"


STATE_DIR = _state_dir()
CA_BUNDLE_PATH = STATE_DIR / "apple_combined_ca.pem"

APPLE_ROOT_CERTS = """
-----BEGIN CERTIFICATE-----

UKqK1drk/NAJBzewdXUh
-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""


def get_apple_ca_bundle_path() -> str:
    """Ensure a CA bundle containing system/certifi roots + Apple Root CAs exists on disk."""
    STATE_DIR.mkdir(parents=True, exist_ok=True)
    if CA_BUNDLE_PATH.exists() and CA_BUNDLE_PATH.stat().st_size > 10000:
        return str(CA_BUNDLE_PATH)

    content = ""
    try:
        import certifi
        c_path = certifi.where()
        if Path(c_path).exists():
            content = Path(c_path).read_text(encoding="utf-8")
    except Exception:
        pass

    if not content:
        if sys.platform == "win32":
            import ssl
            content = ssl.get_default_verify_paths().cafile or ""
            if content and Path(content).exists():
                content = Path(content).read_text(encoding="utf-8")
            else:
                content = ""
        else:
            sys_ca = Path("/etc/ssl/certs/ca-certificates.crt")
            if sys_ca.exists():
                content = sys_ca.read_text(encoding="utf-8")

    combined = content.strip() + "\n\n# Apple Root CAs\n" + APPLE_ROOT_CERTS.strip() + "\n"
    CA_BUNDLE_PATH.write_text(combined, encoding="utf-8")
    log.debug("wrote combined Apple CA bundle to %s", CA_BUNDLE_PATH)
    return str(CA_BUNDLE_PATH)
